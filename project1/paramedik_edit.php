...
$kategori = $dbh->query('SELECT DISTINCT kategori FROM paramedik');
$unit_kerja_id = $dbh->query('SELECT * FROM unit_kerja_id');
...
<div class="container-fluid px-4">
  <h1 class="mt-4">Ubah Paramedik</h1>
  ...
  <input type="hidden" name="id" value="<?= $paramedik['id'] ?>">
      ...
      <div class="col-8">
        <select id="kategori" name="kategori" class="custom-select" required="required">
          ...
          <?php foreach ($kategori as $kat) : ?>
            <option <?= $paramedik['kategori'] == $kat['kategori'] ? 'selected' : '' ?> value="<?= $kat['kategori'] ?>"><?= $kat['kategori'] ?></option>
          <?php endforeach; ?>
        </select>
      </div>
    </div>
    <div class="form-group row">
      ...
      <div class="col-8">
        <input id="telepon" name="telepon" type="text" class="form-control" required="required" value="<?= $paramedik['telepon'] ?>">
      </div>
    </div>
    <div class="form-group row">
    ...
    <div class="form-group row">
      ...
      <div class="col-8">
        <select id="unit_kerja_id" name="unit_kerja_id" class="custom-select" required="required">
          ...
          <?php foreach ($unit_kerja_id as $uk) : ?>
            <option <?= $paramedik['unit_kerja_id'] == $uk['id'] ? 'selected' : '' ?> value="<?= $uk['id'] ?>"><?= $uk['nama'] ?></option>
          <?php endforeach; ?>
        </select>
      </div>
    </div>
